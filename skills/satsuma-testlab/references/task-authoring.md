...
同一 VM 的步骤按数组顺序串行执行，不同 VM 的 Agent 独立执行并可能并发。数组顺序不表示跨 VM 依赖或
就绪关系；当前版本没有服务型步骤或跨 VM readiness barrier。
...
- `execute`/`script` 的根进程必须在超时前返回。Agent 会在根进程结束、超时或取消时收敛整个 Windows Job；
  只终止某一个被测 PID 不足以证明启动脚本已经结束。PowerShell 5.1 的兜底进程树清理可使用
  `taskkill.exe /PID <pid> /T /F`，随后验证退出码和进程状态。
