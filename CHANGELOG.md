...
- Guest 安装介质收敛为单个 `SatsumaVM.exe`：双击并确认 UAC 即可完成安装、更新或状态检查，并在失败时尝试恢复旧版本、配置和服务。
- Host 与 Agent 的 `--version --json`、Agent presence 和发行包清单均记录构建号、运行尝试和 Git 提交，支持核对同版本不同构建。

### 变更

- 发行包仅在 `SatsumaGuestAgent-Install` 交付 `SatsumaVM.exe`；安装器在没有外部 `agent.json` 时生成通用本机配置，消费者自行启动 Host、完成 `check` 后再执行业务任务。
- Artifact 存储按 VM 隔离，并拒绝冲突的目标路径；Guest 证据路径收敛为短路径以兼容传统 Windows 路径长度限制。

### 修复

- 统一首次配置文档的安装位置说明：新安装使用 ProgramData，已确认的历史安装保留原位置。
- 补充独立于使用者项目的安装升级、权限与长路径开发验收夹具，明确自动测试和 UAC 人工界面的验证边界。
- 修复真实崩溃恢复测试计划中不符合当前 Schema 的 echo 字段，并显式以 SYSTEM 执行 Agent 终止夹具，避免误杀普通用户辅助进程；增加计划校验、提前失败诊断和原始 Host 输出保留，用实际 CLI 回归三种故障计划与身份选择。
- 崩溃验收按场景检查最终电源状态：非安全步骤触发人工门禁后的安全关机视为预期结果，仍拒绝其他环境故障。
...
- Host 可根据已授权 VMX 和基础快照使用 `init` 生成本地实验室配置；通用 Guest Agent 在连接后由 Host 登记并获得中性 VM 身份，不再预置业务角色。
...
- 续租取消和交互进程树测试改用不会自行退出的子进程探针，分别验证取消与恢复完成后的退出状态，消除固定延时写标记造成的时序误报。
- CI 不再校验未提交的本机 `lab.json`，并在任意 Schema 校验命令失败时立即停止，避免后续成功命令掩盖错误。
- Windows CI 对 Debug、Release 各执行三轮测试，保留 CTest 日志及续租测试失败证据，便于追踪不稳定问题。
- 发布运行时预建每个步骤的规范结果目录；Host 在 Agent 结果尚未产生时将 Windows 的路径不存在视为正常等待，避免误报诊断结果路径缺失。
