...
// 运行常驻 VMCI 网关。
[[nodiscard]] int handle_gateway(satsuma::LabConfig config) {
...
// 执行实验室租约状态、解锁或崩溃恢复命令。
[[nodiscard]] int handle_lab_command(
    const std::wstring_view subcommand,
...
// 执行运行列表、取消、保留和租约终结命令。
[[nodiscard]] int handle_runs_command(
    const std::wstring_view subcommand,
    const Options& options,
    const std::filesystem::path& config_path,
    ...
    if (subcommand == L"cancel") {
        ...
        auto lease = satsuma::host::LabLease::acquire(
            config, config_path, "runs cancel", run_id, break_lease);
    ...
    auto lease = satsuma::host::LabLease::acquire(config, config_path, "runs prune");
...
// 执行 Agent 环境与 VMCI 通道诊断。
[[nodiscard]] int handle_check_command(
    const Options& options,
    const std::filesystem::path& config_path,
    satsuma::LabConfig config) {
...
// 执行 Agent inventory、身份绑定和二进制更新命令。
[[nodiscard]] int handle_agent_command(
    const std::wstring_view subcommand,
    const Options& options,
    const std::filesystem::path& config_path,
    satsuma::LabConfig config) {
    ...
    auto lease = satsuma::host::LabLease::acquire(
        config,
        config_path,
        satsuma::path_to_utf8(L"agent " + std::wstring(subcommand)));
...
// 执行 VM 启动、停止和快照恢复命令。
[[nodiscard]] int handle_vm_command(
    const std::wstring_view subcommand,
    const Options& options,
    const std::filesystem::path& config_path,
    const satsuma::LabConfig& config) {
    ...
    auto lease = satsuma::host::LabLease::acquire(
        config,
        config_path,
        satsuma::path_to_utf8(L"vm " + std::wstring(subcommand)));
    ...
    nlohmann::json output = {{"status", status}, {"vm_id", vm->id}};
...
// 执行快照查询和受管 AI 快照事务。
[[nodiscard]] int handle_snapshot_command(
    const std::wstring_view subcommand,
    const Options& options,
    const std::filesystem::path& config_path,
    const satsuma::LabConfig& config) {
    ...
    auto lease = satsuma::host::LabLease::acquire(
        config,
        config_path,
        satsuma::path_to_utf8(L"snapshot " + std::wstring(subcommand)));
    if (subcommand == L"delete-ai") {
        const std::string snapshot_name = satsuma::path_to_utf8(
            require_option(options, L"snapshot"));
        try {
...
// 执行带持久生命周期状态的多 VM 编排。
[[nodiscard]] int handle_orchestrate_command(
    const Options& options,
    const std::filesystem::path& config_path,
    satsuma::LabConfig config) {
    const std::filesystem::path plan_path = require_option(options, L"plan");
...
// 诊断涉及的 VM 后发布一次普通任务。
[[nodiscard]] int handle_run_command(
    const Options& options,
    const std::filesystem::path& config_path,
    const satsuma::LabConfig& config) {
    const std::filesystem::path plan_path = require_option(options, L"plan");
...
// 读取运行报告，并可有限等待规范终态。
[[nodiscard]] int handle_report_command(
    const Options& options,
    satsuma::LabConfig config) {
    const std::string run_id = satsuma::path_to_utf8(require_option(options, L"run"));
    const std::optional<std::chrono::seconds> wait = parse_report_wait(options);
    const auto started = std::chrono::steady_clock::now();
    const auto deadline = wait.has_value() ? started + *wait : started;
    satsuma::host::Controller controller(std::move(config));
...
int run_cli(const int argc, wchar_t* argv[]) {
    try {
        if (argc < 2) {
            print_usage();
            return 2;
        ...
        if (command == L"gateway") {
            return handle_gateway(std::move(config));
        }

        if (command == L"lab") {
            return handle_lab_command(subcommand, options, config_path, config);
        }
        ...
        if (command == L"runs") {
            return handle_runs_command(subcommand, options, config_path, std::move(config));
        }

        if (command == L"check") {
            return handle_check_command(options, config_path, std::move(config));
        }

        if (command == L"agent") {
            return handle_agent_command(subcommand, options, config_path, std::move(config));
        }

        if (command == L"vm") {
            return handle_vm_command(subcommand, options, config_path, config);
        }

        if (command == L"snapshot") {
            return handle_snapshot_command(subcommand, options, config_path, config);
        }
        ...
        if (command == L"run") {
            return handle_run_command(options, config_path, config);
        }
        if (command == L"report") {
            return handle_report_command(options, std::move(config));
        }
