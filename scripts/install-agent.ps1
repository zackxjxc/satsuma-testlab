...
param(
    ...
    [switch]$ElevatedWindow
)

$ErrorActionPreference = 'Stop'

# 人工运行保留成功/失败窗口；自动化显式使用 -NonInteractive。
$pauseInElevatedWindow = -not $NonInteractive
trap {
    Write-Host "SatsumaVM 安装失败：$($_.Exception.Message)" -ForegroundColor Red
    if ($pauseInElevatedWindow) {
        [void](Read-Host '按 Enter 关闭此安装窗口')
    }
    exit 1
}
...
# 无外部 JSON 时使用通用 Agent 配置；实验室身份由首次在线登记提供。
function Read-SatsumaInstallConfig {
    param([string]$Path)
    if (Test-Path -LiteralPath $Path -PathType Leaf) {
        return Get-Content -Raw -Encoding UTF8 -LiteralPath $Path | ConvertFrom-Json
    }
    return [pscustomobject]@{ schema_version = 1 }
}
...
if (-not (Test-Administrator)) {
    ...
    $elevatedArguments += '-ElevatedWindow'
    if ($NonInteractive) {
        $elevatedArguments += '-NonInteractive'
    }
    $argumentText = (
        $elevatedArguments |
...
foreach ($sourceFile in @($sourceAgent)) {
...
try {
    ...
    $candidateConfig = Read-SatsumaInstallConfig $sourceConfig
    ...
    $installError = $_
    if (-not $installSucceeded -and $candidateValidated) {
        try {
            Stop-SatsumaService
        } catch {
        ...
        try {
            if ($agentWasBackedUp -or $agentWasInstalled) {
            ...
            if ($configWasBackedUp -or $configWasInstalled) {
...
if ($pauseInElevatedWindow) {
    [void](Read-Host '安装完成，按 Enter 关闭此安装窗口')
}
