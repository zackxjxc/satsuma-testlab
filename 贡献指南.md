...
会改变文件协议、VM 生命周期语义、Service 权限、支持的 VMware 产品或兼容性承诺的变更，应先通过 Issue
...
变更应保持 Shared Folder 作为唯一任务事实源，显式限定 VM 范围，使用原子协议写入，并为取消和超时设置
有限边界。新增依赖必须有明确的维护收益、精确版本，并在 `第三方声明.md` 中登记。

不得提交真实 VM 凭据、VMX 内容、本机绝对路径、Shared Folder 证据、私有 Artifact 或包含用户数据的测试
